...
// function to clear editors from all slides
...
(function ( document, window ) {
    ...
    document.addEventListener("impress:init", function (event) {
        ...
        // If we are in the edit mode, we do not want to navigate, as we
		// were in the normal mode. We want for example the whitespace to do
		// what it is supposed to do when editing text
		var normalMode = true;
		if (((document.location.href).indexOf("?edit/")) != -1) {
			normalMode = false;
		}
		...
		if(!normalMode) {
			...
			$(function() {
				$('#nextBtnEditMode').click(function() {
					//clearEditor();	
					//Aloha.jQuery("#title").find(".fakeClassNameForAloha").blur();
					api.next();
				});
				
				$('#prevBtnEditMode').click(function() {
					//clearEditor();				
					api.prev();
				});
			});
